...
class Model {
	...
	load(deep: boolean, modelLoadCallback) {
		const othis = this;
	...
	create(className: string, object, callback) {
	...
	commit(comment: string, callback) {
		const tid = this.checkTransaction();
	...
	extendClass(wrapperClass, typeName: string) {
		...
		realType.superclasses.forEach((typeName: string) => {
		...
		for (let fieldName in realType.fields) {
			...
			(function (field, fieldName) {
				...
				if (field.reference) {
					wrapperClass["set" + fieldName.charAt(0).toUpperCase() + fieldName.slice(1) + "Wrapped"] = function (typeName: any, value: any) {
					...
					wrapperClass["set" + fieldName.charAt(0).toUpperCase() + fieldName.slice(1)] = function (value: any) {
						const tid = othis.checkTransaction();
						const object = this.object;
					...
					wrapperClass["add" + fieldName.charAt(0).toUpperCase() + fieldName.slice(1)] = function (value, callback) {
						const object = this.object;
						const tid = othis.checkTransaction();
					...
					wrapperClass["remove" + fieldNameFirstUpper] = function (value, callback) {
						const object = this.object;
						const tid = othis.checkTransaction();
					...
					wrapperClass["get" + fieldNameFirstUpper] = function (callback) {
						const object = this.object;
					...
					wrapperClass["get" + fieldNameFirstUpper] = function (callback: any) {
						const object = this.object;
						if (field.many) {
							if (object[fieldName] == null) {
								object[fieldName] = [];
...
//							object[fieldName].push = function () {};
					...
					wrapperClass["set" + fieldNameFirstUpper] = function (value: any) {
						const object = this.object;
						object[fieldName] = value;
						const tid = othis.checkTransaction();
						if (field.many) {
